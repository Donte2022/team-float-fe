...
// Full List Getters and Setters
  ...
  getProductById(id:number) {
    let index: number = this.fullProduct.findIndex(product=>product.id===id)
    console.log(index)
    console.log( this.fullProduct[index])
    return this.fullProduct[index]
  }
...
// Screen Setters
  ...
  deleteProduct (Input: number) {
    let obs = this.http.delete("/product?id="+ Input )
    obs.subscribe({
      next: () => {
  ...
  deletePriceChange (proid: number, priid: number,pro: IProduct) {
    let obs = this.http.delete("/price/" + proid + "/" + priid)
    obs.subscribe({
      next: () => {
  ...
  deleteCategory (input : number) {
    let obs = this.http.delete("/categories/"+input)
    obs.subscribe({
      next: () => {
        for (let pro of this.fullProduct){
